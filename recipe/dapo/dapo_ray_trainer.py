...
from verl.trainer.ppo.metric_utils import compute_data_metrics, compute_throughout_metrics, compute_timing_metrics, process_validation_metrics, reduce_metrics
...
class RayDAPOTrainer(RayPPOTrainer):
    ...
    def fit(self) -> None:
        ...
        self.generation_state = GenerationState(sampling_params={"max_tokens": self.config.actor_rollout_ref.rollout.response_length})
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                self.generation_state.prompt_batch = DataProto.concat([self.generation_state.prompt_batch, new_prompt_batch]) if self.generation_state.prompt_batch is not None else new_prompt_batch

                prompt_bsz = self.config.data.train_batch_size
                if not self.config.algorithm.filter_groups.dynamic_gen_bs.enable:
                    target_num_prompt_needed = prompt_bsz * self.config.algorithm.filter_groups.oversampling_iters
                else:
                    estim_num_prompt_needed = -int(-1 / self.updating_state.qualified_rate) * prompt_bsz if self.updating_state.qualified_rate > 0 else 0
                    estim_num_remaining_prompt_needed = estim_num_prompt_needed - self.updating_state.gen_prompt_cnt
                    target_num_prompt_needed = int(estim_num_remaining_prompt_needed * self.config.algorithm.filter_groups.dynamic_gen_bs.oversampling_factor)
                    logger.info(
                        ...
                        target_num_prompt_needed,
                    )

                if len(self.generation_state.prompt_batch) <= target_num_prompt_needed:
                ...
                with _timer("step", self.updating_state.timing_raw):
                    ...
                    if not self.config.algorithm.filter_groups.enable:
                        ...
                        self.updating_state.batch = DataProto.concat([self.updating_state.batch, new_batch]) if self.updating_state.batch is not None else new_batch

                        # Ceiling
                        self.updating_state.gen_traj_cnt += len(gen_batch_output)
                        self.updating_state.qualified_rate = len(self.updating_state.batch) / self.updating_state.gen_traj_cnt
                        traj_bsz = prompt_bsz * self.config.actor_rollout_ref.rollout.n
                        if len(self.updating_state.batch) < traj_bsz:
                            logger.info(
                                "[step:%d/gen_round_cnt:%d] # of Traj. (%d) < Traj. Batch Size (%d). Keep generating...",
                                self.global_steps,
                                self.updating_state.gen_round_cnt,
                    ...
                    if self.config.data.dynamic_max_resp_len.enable:
                        ...
                        logger.info(
                            "[step:%d/gen_round_cnt:%d] Max non-truncated response length: %d / Max response length: %d",
                            self.global_steps,
                            self.updating_state.gen_round_cnt,
                        ...
                        max_tokens = min(
                            int(max_non_truncated_resp_len * (1 + self.config.data.dynamic_max_resp_len.extending_tolerance)),
                    ...
                    self.updating_state.batch.meta_info["global_token_num"] = torch.sum(self.updating_state.batch.batch["attention_mask"], dim=-1).tolist()
                ...
                # collect metrics
                assert self.updating_state.batch is not None
                self.updating_state.metrics.update(compute_data_metrics(batch=self.updating_state.batch, use_critic=self.use_critic))
                self.updating_state.metrics.update(compute_timing_metrics(batch=self.updating_state.batch, timing_raw=self.updating_state.timing_raw))
                # TODO: implement actual tflpo and theoretical tflpo
                n_gpus = self.resource_pool_manager.get_n_gpus()
                self.updating_state.metrics.update(compute_throughout_metrics(batch=self.updating_state.batch, timing_raw=self.updating_state.timing_raw, n_gpus=n_gpus))
