...
if [[ "${RECIPE}" =~ "da" ]]; then
    ...
    if [[ "${RECIPE}" =~ "orig-data" ]]; then
        ...
        last_user_msg_template="none_template"
    ...
    if [[ "${RECIPE}" =~ "orig-rew" ]]; then
        ...
        custom_reward_fn_src_path=null
        custom_reward_fn_name=null
    fi

    if [[ "${RECIPE}" =~ "repro-micro" ]]; then
        actor_train_max_token_num=$((max_prompt_length + max_response_length))
        infer_max_token_num=$((max_prompt_length + max_response_length))
    fi

    enable_dynamic_gen_bs=False
    [[ "${RECIPE}" =~ "dgbs" ]] && enable_dynamic_gen_bs=True
...
ckpt_name="${RECIPE}-${MODEL_ID}-lr${ACTOR_LR}-bs${TRAIN_BS}-nup${N_UPDATES_PER_BATCH}-fsdp${FSDP_SIZE}-${num_procs}gpus"
[ "${TEST}" == "1" ] && ckpt_name="${ckpt_name}-test"

exp_name="${ckpt_name}-${DEVICE}-$(git rev-parse --short HEAD)-$(date +%Y%m%d-%H%M%S)"

log_home="${RAY_DATA_HOME}/logs/${project_name}/train"
mkdir -p "${log_home}"
...
python3 -m recipe.dapo.main_dapo \
    ...
    algorithm.filter_groups.oversampling_iters=${oversampling_iters} \
    algorithm.filter_groups.dynamic_gen_bs.enable=${enable_dynamic_gen_bs} \
    algorithm.filter_groups.dynamic_gen_bs.oversampling_factor=${oversampling_factor} \
    ...
    actor_rollout_ref.rollout.balance_gen=${balance_gen} \
